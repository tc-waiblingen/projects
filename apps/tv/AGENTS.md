...
### TV Display Text Sizes
Use these centralized classes for TV screen components (`src/app/tv/` and `src/components/tv/`):

| Class | Size | Purpose |
|-------|------|---------|
| `tv-title` | text-6xl | Screen titles in TvScreenLayout |
| `tv-heading` | text-2xl | Section headers, card titles |
| `tv-message` | text-3xl | Empty state messages, announcements |
| `tv-body-lg` | text-xl | Secondary body text, group names |
| `tv-body` | text-lg | Primary body text, descriptions |
| `tv-small` | text-sm | Small text, badges |
| `tv-meta` | text-xs | Metadata, timestamps, details |
| `tv-initials` | text-5xl | Avatar initials fallback |
| `tv-sponsor-name-lg` | text-4xl | Premium sponsor name fallback |
| `tv-sponsor-name` | text-xl | Other sponsor name fallback |

Use these classes instead of raw `text-*` size classes in TV components. This allows global adjustments to TV text sizes in one place (`globals.css`).
