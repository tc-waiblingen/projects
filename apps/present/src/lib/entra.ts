...
export const PKCE_COOKIE = 'present_entra_pkce'
export const STATE_COOKIE = 'present_entra_state'
export const NONCE_COOKIE = 'present_entra_nonce'
export const NEXT_COOKIE = 'present_entra_next'
...
function readEnv(): EntraEnv {
  ...
  return {
    ...
    clientId,
    clientSecret,
    adminRole: process.env.ENTRA_ADMIN_ROLE ?? 'Present.Admin',
    moderatorRole: process.env.ENTRA_MODERATOR_ROLE ?? 'Present.Moderator',
  }
}
...
export async function exchangeCallback(params: {
  ...
  const role: Role = roles.includes(env.adminRole) ? 'admin' : 'moderator'
  if (role !== 'admin' && !roles.includes(env.moderatorRole)) {
    throw new Error('Entra user is missing required Present role')
  }
