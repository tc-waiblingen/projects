    ...
    >
      <div className="self-stretch">
        ...
        <p className="mt-1 inline-flex gap-1 text-base/7">
          ...
          {period && <span className="text-tcw-accent-500 dark:text-tcw-accent-500">{period}</span>}
        </p>
      ...
      <ElTabGroup>
        <Container className="flex flex-col gap-16">
          <div className="flex flex-col items-center gap-6">
            ...
            <ElTabList className="flex items-center gap-1 rounded-full bg-tcw-accent-900/5 p-1 dark:bg-white/5">
              {options.map((option) => (
                <button
                  key={option}
                  type="button"
                  className="rounded-full px-4 py-1 text-sm/7 font-medium text-tcw-accent-900 aria-selected:bg-tcw-accent-900 aria-selected:text-white dark:text-white dark:aria-selected:bg-white/10 dark:aria-selected:text-white"
                >
                  {option}
                </button>
              ))}
