...
export function NavbarLink({
  ...
  return (
    <Link
      href={href}
      className={clsx(
        'group interactive-group inline-flex items-center justify-between gap-2 text-3xl/10 font-medium text-tcw-accent-900 lg:text-sm/7 dark:text-white',
        className,
      )}
      {...props}
    >
      {children}
      <span className="inline-flex p-1.5 opacity-0 group-hover:opacity-100 group-active:opacity-100 group-focus-visible:opacity-100 lg:hidden" aria-hidden="true">
      ...
      <nav>
        <div className="mx-auto flex h-(--scroll-padding-top) max-w-7xl items-center gap-4 px-6 lg:px-10">
          ...
          <div className="flex flex-1 items-center justify-end gap-4">
            ...
            <button
              ...
              className="inline-flex rounded-full p-1.5 text-tcw-accent-900 hover:bg-tcw-accent-900/10 lg:hidden dark:text-white dark:hover:bg-white/10"
            >
        ...
        <ElDialog className="lg:hidden">
          <dialog id="mobile-menu" className="backdrop:bg-transparent">
            <ElDialogPanel className="fixed inset-0 bg-tcw-accent-100 px-6 py-6 lg:px-10 dark:bg-tcw-accent-900">
              <div className="flex justify-end">
                <button
                  command="close"
                  commandfor="mobile-menu"
                  aria-label="Menü öffnen/schließen"
                  className="inline-flex rounded-full p-1.5 text-tcw-accent-900 hover:bg-tcw-accent-900/10 dark:text-white dark:hover:bg-white/10"
                >
                  <svg
