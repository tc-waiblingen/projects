...
export function Feature({
  ...
  return (
    <div
      className={clsx(
        'group grid grid-flow-dense grid-cols-1 gap-2 rounded-lg bg-tcw-accent-900/2.5 p-2 lg:grid-cols-2 dark:bg-white/5',
        className,
      )}
    >
      <div className="flex flex-col justify-between gap-6 p-6 sm:gap-10 sm:p-10 lg:p-6 lg:group-even:col-start-2">
        <div className="text-xl/8 sm:text-2xl/9">
          <h3 className="text-tcw-accent-900 dark:text-white">{headline}</h3>
          <div className="flex flex-col gap-4 text-tcw-accent-500">{subheadline}</div>
        </div>
        {cta}
      </div>
