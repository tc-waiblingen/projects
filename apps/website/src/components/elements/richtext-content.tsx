...
import { RichtextLightboxProvider } from "./richtext-lightbox-provider"
...
export function RichtextContent({ html, fileMetadata }: RichtextContentProps) {
  ...
  const content = root.childNodes.map((node, i) => nodeToReact(node, i))

  if (collectedImages.length === 0) {
    return <div>{content}</div>
  }

  return <RichtextLightboxProvider images={collectedImages}>{content}</RichtextLightboxProvider>
}
